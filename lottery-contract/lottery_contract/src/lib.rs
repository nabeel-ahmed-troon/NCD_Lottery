...
pub struct Lottery{
    ...
    //Ticket per owner
    ticket_per_owner:UnorderedMap<AccountId,UnorderedSet<u64>>,
    //For Airdrop of Ft
...
impl Lottery{
    ...
    pub fn new()->Self{
        let _this = Self{
            ...
            ticket_per_owner:UnorderedMap::new(StorageKeys::TICKETPEROWNER),
            //Ft Airdrop
    ...
    pub fn start_new_lottery(&mut self,ticket_limit:u64,ticket_price:U128,approved_ft:AccountId,nft_contract:AccountId){
        ...
        if self.winner_picked==true{
            ...
            self.ticket_per_owner=UnorderedMap::new(StorageKeys::TICKETPEROWNER);
            self.ticket_id=0;
    ...
    //getting ticket ids that buyer has
    pub fn ticket_owner(&self,account_id:AccountId)->Vec<u8>{
        let res=  if let Some(value)=self.ticket_per_owner.get(&account_id){
              Some(value)
          }
          else{
              None
          ...
          log!("ownerelrer {:#?}",res);
          let c = res.try_to_vec().unwrap();
          c
      }
      //getting all ticket holders and their holding
      pub fn all_ticket_owner(&self){
          let res=&self.ticket_per_owner;
            log!("all ticket owners are : {:#?}",res);
            // let c = res.keys_as_vector();s
            // c
        }
