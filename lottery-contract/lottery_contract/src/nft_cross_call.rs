...
impl LOTTERYCONTRACT for Lottery{
     ...
     fn claim_reward(&mut self,_token_id: TokenId){
         ...
         let id= _token_id + &(self.winning_nft_token_id.to_string());
        ...
        let _token_metadata= TokenMetadata{
         ...
         //Minting Nft 
        //  nft_contract::nft_mint(_token_id, self.winner.clone(), _token_metadata, self.nft_account.clone(), ONE_YOCTO, BASE_GAS)
        //  .then(
       ...
       nft_contract::nft_mint(
             ...
             BASE_GAS*2);
        self.winning_nft_token_id+=1;
            //  assert!(res.then(ext_self::callback_promise_result(env::current_account_id(), 0, BASE_GAS)).);
...
impl FTActionsReceiver for Lottery{
    fn ft_on_transfer(
        ...
        //getting vector of ticket owner

        // let mut _vec= self.ticket_per_owner.get(&env::predecessor_account_id()).unwrap();
        // _vec.insert(&self.ticket_id);
        // self.ticket_per_owner.insert(&env::predecessor_account_id(), &_vec);
