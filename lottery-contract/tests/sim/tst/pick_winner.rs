...
fn pick_winner(){
    ...
    let res= view!(lottery.get_ticket_limit()).unwrap_json_value();
    println!("liimit {}",res);

    let res= view!(lottery.get_ticket_id()).unwrap_json_value();
    println!("tiket id before {}",res);
    for i in 1..6{
        let s=call!(
            demo,
        ...
        println!("errors are {:#?}",s.promise_errors());
    }
    let res= view!(lottery.get_ticket_id()).unwrap_json_value();
    println!("tiket id after {}",res);
    // for i in 1..7{
    //     call!(
    //         root,
    ...
    let res= view!(lottery.get_winner_of_lottery()).unwrap_json_value();

    println!("Winner of Lottery Is : {} ",res);
