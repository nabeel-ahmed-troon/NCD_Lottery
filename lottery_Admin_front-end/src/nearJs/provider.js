...
const config = {
  networkId: "testnet",
  contractName: "ncd_lottery.testnet",
...
export default function NearProvider({ children }) {
  ...
  const isRegistered = useCallback(
    async () => {
      console.log(
        "account_id and contract",
        account.accountId + " " + config.contractName
      );
      if (account == null) {
        throw new Error("Account must be defined");
      }
  ...
  const hasDeposittedForStorage = useCallback(async () => {
    let token;
    if (account != null) {
  ...
  const getTicketPrice = useCallback(async () => {
    ...
    let result = await account?.viewFunction(
      config.contractName,
      "get_ticket_price",
      {}
    );
  ...
  const depositForStorage = useCallback(async () => {
    if (account == null) {
      throw new Error("Account must be defined");
    }
    try {
